...
>
> 🔴 절대 불변 지침: **사지도 못하고 팔지도 못하는 상태를 만들지 않는다.**
> - 어떤 수정/최적화/자동화 변경을 하더라도 `allowSell=true`는 항상 유지.
> - 보유 심볼은 실행 우선순위 최상단에 두고, 매도 기회가 cap 때문에 밀리지 않게 한다.
> - 매수 차단이 걸려도 매도 경로는 항상 열어 둔다.
