...
export function loadConfig(env = process.env) {
  ...
  const runtime = {
    ...
    outLogFile: env.TRADER_OUT_LOG_FILE || env.pm_out_log_path || path.join(cwd, "logs", "trader-out.log"),
    outLogMaxLines: toPositiveInt(env.TRADER_OUT_LOG_MAX_LINES, 10_000),
    outLogPruneCheckEveryWindows: toPositiveInt(env.TRADER_OUT_LOG_PRUNE_CHECK_EVERY_WINDOWS, 300),
  ...
  const strategySettings = {
    ...
    maxAgeSec: toNonNegativeInt(env.STRATEGY_SETTINGS_MAX_AGE_SEC, 0),
    allowSymbolOverride: toBoolean(env.STRATEGY_SETTINGS_ALLOW_SYMBOL_OVERRIDE, false),
  ...
  const optimizer = {
    ...
    liveSafetyGateEnabled: toBoolean(env.OPTIMIZER_LIVE_SAFETY_GATE_ENABLED, false),
    liveSafetyGateMaxAgeSec: toPositiveInt(env.OPTIMIZER_LIVE_SAFETY_GATE_MAX_AGE_SEC, 7200),
    reportFile: env.OPTIMIZER_REPORT_FILE || path.join(cwd, ".trader", "optimizer-report.json"),
    useMarketUniverseSymbols: toBoolean(env.OPTIMIZER_USE_MARKET_UNIVERSE_SYMBOLS, true),
    ...
    strategies: ["mean_reversion", "risk_managed_momentum", "breakout"],
    ...
    walkForwardMinAverageReturnPct: toNumber(env.OPTIMIZER_WALK_FORWARD_MIN_AVG_RETURN_PCT, 0),
    walkForwardMinAverageWinRatePct: toNonNegativeNumber(env.OPTIMIZER_WALK_FORWARD_MIN_AVG_WIN_RATE_PCT, 45),
    walkForwardMinAverageExpectancyKrw: toNumber(env.OPTIMIZER_WALK_FORWARD_MIN_AVG_EXPECTANCY_KRW, 1),
    topResults: 10,
    momentumLookbacks: [],
    volatilityLookbacks: [],
    entryBpsCandidates: [],
